(function($){
    ...
    var geoURL = window.location.href.indexOf('snd.org') === -1 ? 'data/combined.json' : 'http://www.snd.org/assets/2013/snd-membership/combined.json';
    ...
    function drawMap(height){
        ...
        d3.json(geoURL, function(error, data) {
            svg.selectAll(".unit")
    ...
    function addEventListeners(){
        $(window).mousemove(function(e){
            var $c = $infobox.parent(),
                offset = $c.offset(),
                midX = $c.outerWidth() / 2 + offset.left;
    ...
    jQuery.extend( jQuery.fn.dataTableExt.oSort, {
        "emptybottom-pre": function ( x ) {
            return x;
        },
     
        "emptybottom-asc": function ( x, y ) {
            var retVal;
            x = $.trim(x);
            y = $.trim(y);
